...
import dijkstra.{ConnectionDis, DistanceDijkstraImpl, JsonUtil}
import scala.collection.mutable.{LinkedHashMap, LinkedList}
...
class SparkES extends Pools with Serializable {
  ...
  def Search(start: String, end: String): Unit = {
    ...
    val queryt = "{\n" +
      ...
      "  }\n" +
      "}"
    val se = sc.esRDD("gxy/gxy", queryg)
    val sort = se.map(x => (x._2.getOrElse("HPHM", 0) + " " + x._2.getOrElse("JGSK", 0)
      , (x._2.get("HPHM").get, x._2.get("JGSK").get, x._2.get("HPYS").get
      , x._2.get("CLPP").get, x._2.get("CSYS").get, x._2.get("CLLX").get, x._2.get("HPZL").get, x._2.get("KKBH").get)))
